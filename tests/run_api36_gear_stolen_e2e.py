#!/usr/bin/env python3
"""Prove exact CharacterCreate Gear Stolen persistence on an API 36 phone."""
...
CONTROLS = ("CharacterCreate.chkGearStolen",)
ROOT_GEAR_ID = "a7111111-7111-7111-7111-711111111111"
CAREER_GEAR_ID = "a8111111-8111-8111-8111-811111111111"
TARGET_LABEL = "Gear Stolen Root > Gear Stolen Child > Gear Stolen Target · c7111111"
PROOF_KEYS = (
...
def open_stolen_page(device: shared.Device) -> None:
    ...
    device.tap(f"gear-stolen-open-{token}", scroll=True, timeout=120, max_scrolls=36)
    device.wait(f"gear-stolen-page-{token}", timeout=60)


def select_target(device: shared.Device) -> None:
    token = ROOT_GEAR_ID.replace("-", "")
    device.tap(f"gear-stolen-target-{token}", timeout=60, scroll=True)
    device.tap(TARGET_LABEL, timeout=60, scroll=True, max_scrolls=24)
    time.sleep(0.35)
    observed = shared.selected_text(
        device,
        f"gear-stolen-target-{token}",
        "Gear node",
        scroll=True,
    )
    if observed != TARGET_LABEL:
        device.capture("gear-stolen-target-mismatch")
        raise RuntimeError(f"Gear Stolen target was {observed!r}; expected {TARGET_LABEL!r}")


def assert_ui(device: shared.Device, expected: bool) -> None:
    token = ROOT_GEAR_ID.replace("-", "")
    observed = (
        device.wait(f"gear-stolen-toggle-{token}", timeout=60, scroll=True)
        .attributes.get("checked")
        == "true"
    )
    if observed != expected:
        device.capture("gear-stolen-toggle-mismatch")
        raise RuntimeError(f"Gear Stolen was {observed!r}; expected {expected!r}")
...
def assert_workspace(device: shared.Device) -> None:
    ...
    for payload in workspace_payloads(device):
        ...
        if gear is None:
            continue
        target = gear.find("./children/gear/children/gear")
        sibling = next((
            node for node in root.findall("./gears/gear")
            if node.findtext("guid", default="").lower()
            == "d7111111-7111-7111-7111-711111111111"
        ), None)
        ...
        if (
            ...
            and gear.findtext("stolen") == "False"
            and gear.findtext("notes") == "Creation root Gear sentinel"
            and sibling is not None
            and sibling.findtext("stolen") == "False"
            and sibling.findtext("notes") == "Creation untouched Gear sentinel"
            and root.findtext("customstate") == "Creation Gear Stolen runner sentinel"
        ):
            return
    device.capture("gear-stolen-workspace-not-persisted")
    raise RuntimeError(f"Gear Stolen was not durable: {observations!r}")
...
def assert_career_negative(device: shared.Device, fixture: Path) -> None:
    ...
    open_gear_item(device, CAREER_GEAR_ID)
    selector = f"gear-stolen-open-{CAREER_GEAR_ID.replace('-', '')}"
    try:
        device.wait(selector, timeout=5, scroll=True)
    except RuntimeError:
        pass
    else:
        device.capture("gear-stolen-career-action-exposed")
        raise RuntimeError("Create-only Gear Stolen was exposed for a Career runner")
...
def main() -> int:
    ...
    parser.add_argument(
        "--creation-runner",
        type=Path,
        default=fixtures / "creation-gear-stolen-e2e.chum5",
    )
    parser.add_argument(
        "--career-runner",
        type=Path,
        default=fixtures / "career-gear-stolen-negative-e2e.chum5",
    )
    ...
    source_paths = {
        ...
        "gearStolenPageSha256": android_root / "src/Chummer.Android/Native/GearStolenPage.cs",
        "collectionEditorPagesSha256": android_root / "src/Chummer.Android/Native/CollectionEditorPages.cs",
        "coordinatorSha256": android_root / "src/Chummer.Android/Native/RunnerSessionCoordinator.cs",
        "gearStolenContractSha256": overview / "GearStolenEditRequest.cs",
        ...
        "gearStolenRulesSha256": contracts / "CharacterGearStolenRules.cs",
    ...
    if missing:
        raise RuntimeError(f"Gear Stolen source graph is incomplete: {missing!r}")
    ...
    if api != "36":
        raise RuntimeError(f"Gear Stolen E2E requires API 36, got {api!r}")
    abi_list = device.shell("getprop", "ro.product.cpu.abilist")
    if "arm64-v8a" not in abi_list.split(","):
        raise RuntimeError(f"Gear Stolen E2E requires arm64-v8a, got {abi_list!r}")
    subprocess.run(
    ...
    token = ROOT_GEAR_ID.replace("-", "")
    device.tap(f"gear-stolen-toggle-{token}", timeout=60, scroll=True)
    assert_ui(device, True)
    device.tap(f"gear-stolen-save-{token}", timeout=180, scroll=True, max_scrolls=24)
...
if __name__ == "__main__":
    try:
        ...
        print(f"Gear Stolen E2E failed: {error}", file=sys.stderr)
        raise SystemExit(1)
