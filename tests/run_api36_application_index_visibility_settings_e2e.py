#!/usr/bin/env python3
"""Prove Chummer5 index-visibility Global Settings semantics on an API 36 phone."""
...
def open_settings(device: shared.Device) -> None:
    ...
    device.wait("settings-hide-master-index", timeout=45, scroll=True, max_scrolls=16)
...
def assert_visibility_state(
    device: shared.Device,
    revision: int,
    hide_master_index: bool,
    hide_character_roster: bool,
) -> None:
    state = read_state(device)
    observed = (
        state.get("Revision"),
        state.get("HideMasterIndex"),
        state.get("HideCharacterRoster"),
    )
    expected = (revision, hide_master_index, hide_character_roster)
    if observed != expected:
        raise RuntimeError(f"Application index-visibility snapshot mismatch: {state!r}")
...
def main() -> int:
    ...
    parser.add_argument(
        "--runner",
        type=Path,
        default=Path(__file__).resolve().parent / "fixtures/application-index-visibility-settings-e2e.chum5",
    )
    ...
    if checked(device, "settings-hide-master-index") or checked(device, "settings-hide-character-roster"):
        raise RuntimeError("Chummer5 hidemasterindex/hidecharacterroster defaults must both be false")

    device.tap("settings-hide-master-index", timeout=45, scroll=True)
    device.tap("settings-hide-character-roster", timeout=45, scroll=True)
    device.back()
    open_settings(device)
    if checked(device, "settings-hide-master-index") or checked(device, "settings-hide-character-roster"):
        raise RuntimeError("Back persisted index-visibility drafts")
    if find_state_paths(device):
        raise RuntimeError("Draft-only index-visibility edits created durable state")

    device.tap("settings-hide-master-index", timeout=45, scroll=True)
    device.tap("settings-hide-character-roster", timeout=45, scroll=True)
    device.tap("settings-save", timeout=45, scroll=True)
    assert_visibility_state(device, 1, True, True)
    primary = state_path(device)

    device.shell("am", "force-stop", shared.PACKAGE)
    shared.launch_app(device)
    open_settings(device)
    if not checked(device, "settings-hide-master-index") or not checked(
        device, "settings-hide-character-roster"
    ):
        raise RuntimeError("Both visibility values did not survive process restart")

    device.tap("settings-hide-master-index", timeout=45, scroll=True)
    device.tap("settings-save", timeout=45, scroll=True)
    assert_visibility_state(device, 2, False, True)
    backup = read_state(device, primary + ".bak")
    if (
        backup.get("Revision"),
        backup.get("HideMasterIndex"),
        backup.get("HideCharacterRoster"),
    ) != (1, True, True):
    ...
    shared.launch_app(device)
    open_settings(device)
    if checked(device, "settings-hide-master-index") or not checked(
        device, "settings-hide-character-roster"
    ):
        raise RuntimeError("Independent visibility values did not survive the second process restart")
    ...
    device.capture("application-index-visibility-settings-recovery")
...
if __name__ == "__main__":
    try:
        ...
        print(f"application-index-visibility-settings E2E failed: {error}", file=sys.stderr)
        raise SystemExit(1)
