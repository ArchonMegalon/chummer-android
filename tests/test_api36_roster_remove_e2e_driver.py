...
DRIVER = REPO / "tests/run_api36_roster_remove_e2e.py"
FIXTURE = REPO / "tests/fixtures/roster-remove-e2e.chum5"
...
class Api36RosterRemoveDriverTests(unittest.TestCase):
    def test_driver_is_phone_only_api36_arm64_package_and_digest_bound(self) -> None:
        source = DRIVER.read_text(encoding="utf-8")
        ast.parse(source)
        self.assertIn('CONTROL = "tsDelete"', source)
        self.assertIn('api != "36"', source)
        self.assertIn('abi != "arm64-v8a"', source)
        self.assertIn('"profile": "phone"', source)
        self.assertIn('"journey": "roster-remove"', source)
    ...
    def test_typed_shared_seam_and_both_phone_targets_are_present(self) -> None:
        ...
        store = (
            CORE / "Chummer.Infrastructure/Files/FileCharacterRosterFavoriteStore.cs"
        ).read_text(encoding="utf-8")
        self.assertIn("enum CharacterRosterRemoveTarget", contract)
        self.assertIn("record CharacterRosterRemoveMutation", contract)
        self.assertIn("CharacterRosterRemoveTarget.Favorites", rules)
        self.assertIn("CharacterRosterRemoveTarget.Recent", rules)
        self.assertIn("mutation.ExpectedRevision != current.Revision", rules)
        self.assertIn("CharacterRosterFavoriteRules.ApplyRemove", presenter)
        self.assertIn("_store.Save(mutation.ExpectedRevision, updated)", presenter)
        self.assertIn('AutomationId = "roster-remove-favorite"', page)
        self.assertIn('AutomationId = "roster-remove-recent"', page)
        self.assertIn("_rosterFavoritePresenter.ApplyRemove", coordinator)
        self.assertIn("Flush(flushToDisk: true)", store)
        self.assertIn("File.Replace", store)
        self.assertIn('path + ".bak"', store)

    def test_fixture_is_public_minimal_and_character_xml_is_not_roster_storage(self) -> None:
        root = ET.parse(FIXTURE).getroot()
        self.assertEqual("Remove Proof", root.findtext("alias"))
        self.assertEqual(
            "character XML must remain unrelated",
            root.findtext("./customstate/rosterremove"),
        )
    ...
    def test_inventory_and_receipt_validation_fail_closed(self) -> None:
        spec = importlib.util.spec_from_file_location("inventory_roster_remove", INVENTORY_SCRIPT)
        assert spec is not None and spec.loader is not None
        inventory = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(inventory)
        specs = inventory._capture_only_phone_e2e_specs(PRESENTATION, CORE)
        roster = specs["roster-remove"]
        ...
        rows = [
            ...
            and row["legacy"]["controlName"] == "tsDelete"
        ]
