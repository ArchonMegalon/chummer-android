...
class Api36EditingE2EWorkflowTests(unittest.TestCase):
    ...
    def test_builds_a_separate_bound_arm64_physical_install_authority(self) -> None:
        ...
        self.assertIn("apksigner did not report exactly one signer", arm64)
        self.assertIn("APK has no verified v2 or v3 signature scheme", arm64)
        self.assertIn("extract_apksigner_certificate_sha256.py", arm64)
        self.assertIn("signing certificate SHA-256 extraction failed", arm64)
        self.assertIn("verified signing certificate SHA-256 length:", arm64)
    ...
    def test_executes_every_persistence_driver_as_an_isolated_matrix_journey(self) -> None:
        ...
        journeys = (
            ...
            (
                "career-notoriety",
                "tests/run_api36_career_notoriety_e2e.py",
            ),
        )
        self.assertIn(
        ...
        self.assertEqual(
            ...
            self.text.count(
                ...
                "matrix.journey == 'career-notoriety' }}"
            ),
        )
    ...
    def test_aggregate_requires_five_stable_authority_bound_receipts(self) -> None:
