#!/usr/bin/env python3
"""Prove Chummer5 CharacterRoster Sort on a real API 36 phone."""
...
CONTROL_PROOF_KEYS = (
    ...
    "characterXmlUnchanged",
)
UNSORTED_FAVORITES = ("content://runner/zed", "content://runner/alpha")
SORTED_FAVORITES = tuple(sorted(UNSORTED_FAVORITES))
UNSORTED_RECENT = ("content://runner/yankee", "content://runner/bravo")
SORTED_RECENT = tuple(sorted(UNSORTED_RECENT))


def open_roster(device: shared.Device, sort_control: str) -> None:
    ...
    if sort_control not in {"roster-sort-favorites", "roster-sort-recent"}:
        raise RuntimeError(f"Unknown roster sort control {sort_control!r}")
    device.wait(sort_control, timeout=45, scroll=True, max_scrolls=12)
...
def seed_unsorted_state(device: shared.Device, primary: str) -> None:
    state = {
        "Revision": 7,
        "Favorites": [
            {"Locator": UNSORTED_FAVORITES[0], "DisplayName": "Alpha display"},
            {"Locator": UNSORTED_FAVORITES[1], "DisplayName": "Zulu display"},
        ],
        "Recent": [
            {"Locator": UNSORTED_RECENT[0], "DisplayName": "Beta display"},
            {"Locator": UNSORTED_RECENT[1], "DisplayName": "Omega display"},
        ],
    }
...
def assert_state(
    ...
    *,
    ...
    def locators(key: str) -> tuple[str, ...]:
        items = state.get(key)
        if not isinstance(items, list) or not all(isinstance(item, dict) for item in items):
            raise RuntimeError(f"Roster {key} collection is invalid: {state!r}")
        return tuple(str(item.get("Locator", "")) for item in items)

    if locators("Favorites") != favorites or locators("Recent") != recent:
        raise RuntimeError(f"Roster collection order mismatch: {state!r}")
...
def main() -> int:
    ...
    parser.add_argument(
        "--runner",
        type=Path,
        default=Path(__file__).resolve().parent / "fixtures/roster-sort-e2e.chum5",
    )
    ...
    if missing:
        raise RuntimeError(f"Roster-sort source graph is incomplete: {missing!r}")
    ...
    if api != "36":
        raise RuntimeError(f"Roster-sort E2E requires API 36, got {api!r}")
    abi = device.shell("getprop", "ro.product.cpu.abi")
    if abi != "arm64-v8a":
        raise RuntimeError(f"Roster-sort E2E requires arm64-v8a, got {abi!r}")
    subprocess.run(
    ...
    seed_unsorted_state(device, primary)
    shared.launch_app(device)
    open_roster(device, "roster-sort-favorites")
    assert_state(
        device,
        revision=7,
        favorites=UNSORTED_FAVORITES,
        recent=UNSORTED_RECENT,
    )

    device.tap("roster-sort-favorites", scroll=True)
    assert_state(
        device,
        revision=8,
    ...
    device.capture("roster-sort-favorites")
    device.back()
    open_roster(device, "roster-sort-favorites")
    assert_state(
        device,
    ...
    device.tap("roster-sort-recent", scroll=True)
    assert_state(
        device,
        revision=9,
        favorites=SORTED_FAVORITES,
        recent=SORTED_RECENT,
    )
    device.capture("roster-sort-recent")
    device.shell("am", "force-stop", shared.PACKAGE)
    shared.launch_app(device)
    open_roster(device, "roster-sort-recent")
    assert_state(
        device,
        revision=9,
        favorites=SORTED_FAVORITES,
    ...
    if shared.sha256(runner) != original_runner_sha256 or current_remote_sha256 != remote_runner_sha256:
        raise RuntimeError("Character XML changed while sorting application roster metadata")
    device.capture("roster-sort-backup-recovery")
...
if __name__ == "__main__":
    try:
        ...
        print(f"roster-sort E2E failed: {error}", file=sys.stderr)
        raise SystemExit(1)
