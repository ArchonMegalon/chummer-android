...
namespace Chummer.Android.Native
{
    ...
    internal sealed class DamageMemoryBackend : ISr5CareerCheckpointBackend
    {
        public string Payload { get; set; } = string.Empty;
        public bool FailReadBack { get; init; }

        public string Read()
            => FailReadBack && Payload.Length > 0 ? Payload + " " : Payload;
        public void Write(string payload) => Payload = payload;
        public void Remove() => Payload = string.Empty;
    }

    internal sealed class DamageOwnerBackend : ISr5CareerCheckpointBackend
    {
        public string Payload { get; set; } = string.Empty;
