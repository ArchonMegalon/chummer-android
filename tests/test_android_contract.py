...
class AndroidContractTests(unittest.TestCase):
    ...
    def test_android_navigation_is_compact_and_app_owned(self) -> None:
        ...
        self.assertIn("private readonly ContentView _routeMarkerHost", build)
        self.assertIn("page.Add(_routeMarkerHost);", build)
        self.assertIn("Grid.SetRow(bodyScroll, 1);", build)
        self.assertIn("_routeMarkerHost.Content = persistedReceipt is null", build)
        self.assertNotIn("_body.Add(persistedReceipt is null", build)
        ...
        self.assertIn('new("phone-runner-sheet", "Career runner")', build)
        self.assertEqual(1, build.count("AddRouteMarker(routeMarker.AutomationId, routeMarker.Label)"))
        self.assertNotIn('AutomationId = "phone-runner-empty"', build)
    ...
    def test_release_automation_is_fail_closed(self) -> None:
        ...
        self.assertIn("UseSharedCompilation=false", build)
        self.assertIn("BuildInParallel=false", build)
        self.assertIn("DOTNET_CLI_USE_MSBUILD_SERVER=0", build)
        self.assertIn("MSBUILDDISABLENODEREUSE=1", build)
        self.assertIn('-p:AndroidSdkDirectory="$AndroidSdkDirectory"', build)
        self.assertIn('-p:JavaSdkDirectory="$JavaSdkDirectory"', build)
        self.assertIn('--output "$staged_publish_dir"', build)
        self.assertIn('-p:PublishDir="$staged_publish_dir/"', build)
        ...
        self.assertIn("CHUMMER_ANDROID_REVISION", source_graph)
        self.assertIn("CHUMMER_PRESENTATION_REVISION", source_graph)
        self.assertIn("CHUMMER_CORE_ENGINE_REVISION", source_graph)
        self.assertIn("CHUMMER_DESIGN_REVISION", source_graph)
        self.assertIn("os.O_EXCL", source_graph)
        self.assertIn("release source repository authority drifted", source_graph)
        self.assertIn("ChummerAndroidSigningStorePass", build)
        self.assertNotIn("-p:AndroidSigningStorePass", build)
        self.assertNotIn('printf "%s" "$ChummerAndroidSigningStorePass"', build)
