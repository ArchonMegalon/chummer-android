...
""",
    ...
    def test_generated_inventory_is_row_complete_unique_and_honestly_incomplete(self) -> None:
        ...
        self.assertEqual("scripted_not_executed", dynamic_condition["e2e"]["phone"]["status"])
        self.assertEqual("scripted_not_executed", dynamic_condition["e2e"]["tablet"]["status"])
        ...
        self.assertFalse(dynamic_condition["completionProven"])
        ...
        self.assertTrue(
            all(
                ...
                for row in scripted_condition_rows
            )
        )
        self.assertTrue(
            all(
                row["e2e"]["tablet"]["status"] == "scripted_not_executed"
                for row in scripted_condition_rows
            )
        )
        self.assertTrue(all(not row["completionProven"] for row in scripted_condition_rows))
        self.assertTrue(all(row["overallStatus"] == "missing" for row in scripted_condition_rows))
        ...
        self.assertEqual(
            {
                "implemented_pending_emulator": 556,
                "missing": 917,
        ...
        self.assertEqual(
            {
                "implemented_pending_emulator": 79,
                "missing": 1528,
                "not_applicable_non_mutating": 478,
    ...
    def test_career_mugshot_mapping_is_two_row_career_only_typed_and_scripted(self) -> None:
        import inspect
        ...
        rows = [
            row for row in payload["rows"]
            if row["legacy"]["formOrControl"] == "CharacterCareer"
            and row["legacy"]["controlName"] in inventory.CAREER_MUGSHOT_CONTROLS
        ]
        self.assertEqual(2, len(rows))
        parameters = list(inspect.signature(inventory._known_phone_mapping).parameters)
        receipt_arguments = {
        ...
        self.assertEqual("career-mugshot-main", mappings["chkIsMainMugshot"]["automationId"])
        self.assertIn("position-and-image-digest", mappings["chkIsMainMugshot"]["presenterMutation"])
        drifted_digests = dict(inventory.CAREER_MUGSHOT_LEGACY_METHOD_DIGESTS)
    ...
    def test_creation_mugshot_delete_is_source_callee_order_guarded_typed_and_scripted(self) -> None:
        import inspect
        ...
        rows = [
            row for row in payload["rows"]
            if row["legacy"]["formOrControl"] == "CharacterCreate"
            and row["legacy"]["controlName"] == inventory.CREATION_MUGSHOT_DELETE_CONTROL
        ]
        self.assertEqual(1, len(rows))
        parameters = list(inspect.signature(inventory._known_phone_mapping).parameters)
        receipt_arguments = {
        ...
        self.assertEqual("CreationMugshotPage", mapping["surface"])
        self.assertEqual("Build > Runner > Creation Mugshots (Creation only)", mapping["route"])
        self.assertEqual("creation-mugshot-delete", mapping["automationId"])
        ...
        drifted_digests = dict(inventory.CREATION_MUGSHOT_DELETE_LEGACY_METHOD_DIGESTS)
        drifted_digests["RemoveMugshot"] = "0" * 64
        with patch.dict(
            inventory.CREATION_MUGSHOT_DELETE_LEGACY_METHOD_DIGESTS,
            drifted_digests,
            clear=True,
        ):
            drifted = inventory._known_phone_mapping(
        ...
        with patch.object(
            ...
            tuple(reversed(inventory.CREATION_MUGSHOT_DELETE_HANDLER_ORDER)),
        ):
            reordered = inventory._known_phone_mapping(
                rows[0],
                inventory.DEFAULT_CHUMMER5_ROOT,
                PRESENTATION_ROOT,
                CORE_ROOT,
        ...
        self.assertEqual("missing", reordered["status"])

        with patch.object(
            inventory,
            "CREATION_MUGSHOT_DELETE_CALLEE_ORDER",
            tuple(reversed(inventory.CREATION_MUGSHOT_DELETE_CALLEE_ORDER)),
        ):
            reordered = inventory._known_phone_mapping(
                rows[0],
                inventory.DEFAULT_CHUMMER5_ROOT,
