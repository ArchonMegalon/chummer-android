...
internal static class AfterRunAuthorityHarness
{
    ...
    private static async Task OwnedAppliedRecoverySurvivesSettledCatalogRevisionAsync()
    {
        ...
        Require(!foreignStore.TryReadOwnedRecordedReceipt(out _, out _),
            "Historical display crossed the local owner boundary.");
        ...
        Require(store.TryReadOwnedRecordedReceipt(out var recorded, out blocker), blocker);
        Require(recorded.Receipt?.ReceiptDigest == applied.Receipt?.ReceiptDigest
            && recorded.Draft.ExpectedWorkspaceRevision == 41,
            "Historical display rebound the old receipt to the new revision.");
        string retained = checkpointBackend.Payload;
        foreach (var invalid in new[]
        {
            Binding(41),
            Binding(43) with { IsDirty = true },
            Binding(43) with { SavedRevision = 42 },
            Binding(43) with { Error = "Reload failed" },
            Binding(43) with { Created = false },
            Binding(43) with { GameEdition = "SR6" },
            Binding(43) with { WorkspaceId = new CharacterWorkspaceId("foreign-runner") }
        })
        {
            presenter.Binding = invalid;
            Require(!store.TryReadOwnedRecordedReceipt(out _, out _),
                "An invalid runner admitted historical receipt display.");
            Require(!store.TryDeleteApplied(Sr5AfterRunSettlementCheckpointCas.From(applied), out _),
                "An invalid runner admitted receipt acknowledgment.");
            Require(checkpointBackend.Payload == retained, "Invalid history access changed the checkpoint.");
        }
        presenter.Binding = Binding(43);
        Require(!authority.OwnsRecordedReceipt(reviewed) && !authority.OwnsRecordedReceipt(applying),
            "Unresolved work was admitted as historical receipt authority.");
        Require(!store.TryDeleteApplied(Sr5AfterRunSettlementCheckpointCas.From(applied) with { Version = applied.Version + 1 }, out _),
            "A stale history action bypassed checkpoint CAS.");
        Require(store.TryDeleteApplied(Sr5AfterRunSettlementCheckpointCas.From(applied), out blocker), blocker);
        Require(checkpointBackend.Payload.Length == 0, "Later saved revisions stranded the acknowledged receipt.");
    }
    ...
    private static async Task AppliedAcknowledgementCannotOrphanUnreleasedOwnershipAsync()
    {
        ...
        var backend = new MemoryBackend();
        var ownerBackend = new MemoryBackend();
        var authority = new Sr5AfterRunSettlementLiveCheckpointAuthority(owner, editor, () => presenter.Binding);
        var sharedOwners = new Sr5CareerMutationOwnerStore(ownerBackend);
        var store = new Sr5AfterRunSettlementCheckpointStore(backend, authority, sharedOwners);
        Require(store.TryCreate(Sr5AfterRunSettlementCheckpoint.FromDraft(draft), out var reviewed, out var blocker), blocker);
        Require(store.TryBeginApply(Sr5AfterRunSettlementCheckpointCas.From(reviewed), out var applying, out blocker), blocker);
        var result = await new Sr5AfterRunSettlementCoordinator(presenter, owner).ApplyAsync(draft, applying, store);
        ownerBackend.FailRemove = true;
        Require(!store.TryRecordAuthoritativeResolution(Sr5AfterRunSettlementCheckpointCas.From(applying),
            result.Resolution, out _, out blocker), "Unacknowledged owner release was reported successful.");
        var applied = JsonSerializer.Deserialize<Sr5AfterRunSettlementCheckpoint>(backend.Payload)!;
        Require(applied.Phase == Sr5CareerCheckpointPhase.Applied && applied.IsStructurallyValid(),
            "Fixture did not retain the applied receipt after release failure.");
        string retained = backend.Payload;
        string shared = ownerBackend.Payload;
        Require(store.TryReadOwnedRecordedReceipt(out _, out blocker), blocker);
        Require(ownerBackend.Payload == shared && backend.Payload == retained,
            "Historical receipt observation silently released shared ownership.");
        Require(!store.TryDeleteApplied(Sr5AfterRunSettlementCheckpointCas.From(applied), out blocker),
            "Acknowledging the receipt deleted the only recovery checkpoint while its shared owner remained.");
        Require(backend.Payload == retained && ownerBackend.Payload == shared,
            "Failed acknowledgment altered recovery authority.");
        ownerBackend.FailRemove = false;
        Require(store.TryReadOwnedRecovery(out applied, out blocker), blocker);
        Require(ownerBackend.Payload.Length == 0, "Exact resolved owner did not reconcile.");
        foreach (string foreign in new[]
        {
            "{}",
            JsonSerializer.Serialize(new Sr5CareerMutationOwner(1, "active-skill-advance",
                WorkspaceId.Value, OwnerId, Guid.NewGuid(), 2, 42, new string('a', 64)))
        })
        {
            ownerBackend.Payload = foreign;
            Require(!store.TryDeleteApplied(Sr5AfterRunSettlementCheckpointCas.From(applied), out _),
                "Corrupt or foreign ownership admitted receipt acknowledgment.");
            Require(backend.Payload == retained && ownerBackend.Payload == foreign,
                "Acknowledgment altered another lane's authority.");
        }
        ownerBackend.Payload = string.Empty;
        bool gateWasHeld = false;
        backend.BeforeRemove = () => gateWasHeld = !sharedOwners.TryRunWhenUnowned(
            () => (true, string.Empty), out _);
        Require(store.TryDeleteApplied(Sr5AfterRunSettlementCheckpointCas.From(applied), out blocker), blocker);
        Require(backend.Payload.Length == 0, "Acknowledged checkpoint was not retired.");
        Require(gateWasHeld, "Another Career transition could race the checkpoint removal.");
    }
    ...
    private sealed class MemoryBackend : ISr5CareerCheckpointBackend
    {
        ...
        public void Remove()
        {
            if (FailRemove) throw new IOException("Injected owner release failure.");
            BeforeRemove?.Invoke();
