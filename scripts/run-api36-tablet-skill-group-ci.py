...
def capture_environment(device, checked: Preflight, environment: Mapping[str, str], sdk: Path,
                        evidence: Path, *, kvm_path: Path = Path("/dev/kvm")) -> tuple[dict, list[authority.StableFile]]:
    ...
    java_home = canonical_directory(environment.get("JAVA_HOME", ""), "Governed Java home")
    java = authority._run([str(java_home / "bin/java"), "-version"])
    ...
    return {"schema": "chummer.android.hosted-tablet-environment/v1", "runnerOs": "Linux", "runnerArch": "X64",
            ...
            "avd": avd_observation, "android": observed}, snapshots
