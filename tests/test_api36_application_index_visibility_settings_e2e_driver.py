...
DRIVER = REPO / "tests/run_api36_application_index_visibility_settings_e2e.py"
FIXTURE = REPO / "tests/fixtures/application-index-visibility-settings-e2e.chum5"
...
class Api36ApplicationIndexVisibilitySettingsDriverTests(unittest.TestCase):
    def test_driver_is_phone_only_api36_arm64_digest_bound_and_not_a_receipt(self) -> None:
        ...
        self.assertIn('"journey": "application-index-visibility-settings"', source)
    ...
    def test_desktop_navigation_values_remain_readable_but_are_not_exposed_on_phone(self) -> None:
        ...
        self.assertIn("bool HideMasterIndex = false", contract)
        self.assertIn("bool HideCharacterRoster = false", contract)
        self.assertIn('LegacyHideMasterIndexIdentity = "hidemasterindex"', rules)
        self.assertIn('LegacyHideCharacterRosterIdentity = "hidecharacterroster"', rules)
        self.assertIn(
    ...
    def test_exact_canonical_default_load_save_authority_is_fail_closed(self) -> None:
        spec = importlib.util.spec_from_file_location("inventory_index_visibility", INVENTORY_SCRIPT)
        assert spec is not None and spec.loader is not None
        inventory = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(inventory)

        self.assertTrue(inventory._application_index_visibility_legacy_authority(CHUMMER5))
        authority = inventory.APPLICATION_INDEX_VISIBILITY_LEGACY_AUTHORITY
        drifted = {
            **authority,
            "methodDigests": {
                **authority["methodDigests"],
                ("Chummer/Forms/EditGlobalSettings.cs", "PopulateOptions"): "0" * 64,
            },
        }
        with patch.object(inventory, "APPLICATION_INDEX_VISIBILITY_LEGACY_AUTHORITY", drifted):
            self.assertFalse(inventory._application_index_visibility_legacy_authority(CHUMMER5))

    def test_fixture_is_public_minimal_and_character_xml_is_not_settings_storage(self) -> None:
        root = ET.parse(FIXTURE).getroot()
        self.assertEqual("Application Index Visibility Proof", root.findtext("alias"))
        self.assertEqual(
            "character XML must remain unrelated",
            root.findtext("./customstate/indexvisibility"),
        )
    ...
    def test_inventory_and_receipt_validation_fail_closed_for_exact_two_rows(self) -> None:
        ...
        settings = specs["application-index-visibility-settings"]
        ...
        for path in (
            ...
            "tests/run_api36_application_index_visibility_settings_e2e.py",
            "tests/fixtures/application-index-visibility-settings-e2e.chum5",
        ):
