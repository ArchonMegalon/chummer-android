            ...
            foreach (string language in new[] { "en", "de", "es" })
            {
                ...
                try
                {
                            ...
                            new("attributes", "A"), new("skills", "B"), new("resources", "C")])
                    {
                        ...
                        Skills = new([]), Karma = new([], [], 5)
                    };
                    var seedResult = await runtime.Coordinator.PreviewSr6FoundationAsync(initial, selection);
                    Require(seedResult.Value is not null, "Gear seed rejected: " + string.Join(",", seedResult.Blockers));
                    var seed = seedResult.Value!;
                    await runtime.Coordinator.ConfirmSr6FoundationAsync(seed, true);
                    var page = new Sr6CreationFoundationPage(runtime.Coordinator, gearMode: true);
                    var navigation = new NavigationPage(page);
                    var window = new Window(navigation);
                    using var alerts = new IssuedPageAlerts(page, window);
                    await alerts.PreflightAsync();
                    ...
                    T Element<T>(string key) where T : Element => IssuedElements(page).OfType<T>().Single(item => item.AutomationId == key);
                    Task Click(string key) => ui.BeginAsyncVoid(() => ((IButtonController)Element<Button>(key)).SendClicked());
                    void Choose(string catalogId)
                    {
                        Element<Entry>("sr6-gear-search").Text = catalogId;
                        Require(Element<Picker>("sr6-gear-catalog").Items.Count == 1, "Gear filter lost exact identity.");
                        Element<Picker>("sr6-gear-catalog").SelectedIndex = 0;
                    }
                    void Add(string catalogId, string quantity)
                    {
                        Choose(catalogId);
                        Element<Entry>("sr6-gear-new-quantity").Text = quantity;
                        ((IButtonController)Element<Button>("sr6-gear-add")).SendClicked();
                    }
                    Require(page.Title == Sr6CreationCopy.Text("GearTitle") && page.AutomationId == "sr6-gear-page", "Gear page localization missing.");
                    Choose("ares-red-dog");
                    Require(!Element<Button>("sr6-gear-add").IsEnabled
                        && Element<Label>("sr6-gear-details").Text.Contains(Sr6CreationCopy.Text("GearAvailabilityExceeded"), StringComparison.Ordinal),
                        "Unavailable gear lost its Core blocker.");
                    Add("lined-coat", "2");
                    await Click("sr6-foundation-preview");
                    var oldConfirm = Element<Button>("sr6-foundation-confirm");
                    Add("credstick-standard", "2");
                    oldConfirm.IsEnabled = true;
                    await ui.BeginAsyncVoid(() => ((IButtonController)oldConfirm).SendClicked());
                    Require(probe!.Confirms == 1, "Basket add reused an obsolete quote.");
                    await Click("sr6-foundation-preview");
                    await Click("sr6-foundation-confirm");
                    var stored = new FileWorkspaceStore(runtime.StateDirectory).Get(id).Value!;
                    var quote = stored.Document.AuxiliaryState.Sr6CreationFoundationDecisions!.Last().Preview;
                    Require(stored.ContentRevision == 3 && stored.SavedRevision == 3 && quote.Gear!.SpentNuyen == 1991m && probe.Confirms == 2,
                        "Native basket lost exact Core decimal price or duplicated save.");
                    Require(quote.Karma!.KarmaSpent == seed.Karma!.KarmaSpent && quote.PointBuy?.PointsSpent == seed.PointBuy?.PointsSpent,
                        "Basket changed Karma or character-point pools.");
                    Require(IssuedElements(page).OfType<Label>().Any(item => item.Text == Sr6CreationCopy.GearBudget(quote.Gear!)), "Saved purchase budget missing.");
                    string coat = quote.Gear!.Items.Single(item => item.Choice.CatalogId == "lined-coat").Choice.Id.ToString("N");
                    string stick = quote.Gear.Items.Single(item => item.Choice.CatalogId == "credstick-standard").Choice.Id.ToString("N");
                    foreach (string invalid in new[] { "0", "1000", "9999999999999999", "1.5", "-1", "" })
                    {
                        Element<Entry>("sr6-gear-quantity-" + coat).Text = invalid;
                        await Click("sr6-foundation-preview");
                        Require(!IssuedElements(page).OfType<Button>().Any(item => item.AutomationId == "sr6-foundation-confirm")
                            && Element<Label>("sr6-foundation-status").Text == Sr6CreationCopy.Text("GearInvalid") && probe.Confirms == 2,
                            "Malformed quantity was not rejected with its gear-specific error: " + invalid + " / " + Element<Label>("sr6-foundation-status").Text);
                    }
                    Element<Entry>("sr6-gear-quantity-" + coat).Text = "2";
                    await Click("sr6-foundation-preview");
                    oldConfirm = Element<Button>("sr6-foundation-confirm");
                    Element<Entry>("sr6-gear-quantity-" + coat).Text = "3";
                    oldConfirm.IsEnabled = true;
                    await ui.BeginAsyncVoid(() => ((IButtonController)oldConfirm).SendClicked());
                    Require(probe.Confirms == 2, "Quantity edit reused old confirmation.");
                    ((IButtonController)Element<Button>("sr6-gear-remove-" + stick)).SendClicked();
                    await Click("sr6-foundation-preview");
                    await Click("sr6-foundation-confirm");
                    ...
                    Require(stored.SavedRevision == 4 && quote.Gear!.Items.Single().Choice.Id.ToString("N") == coat
                        && quote.Gear.Items.Single().Choice.Quantity == 3 && quote.Gear.SpentNuyen == 2970m, "Basket edit/remove lost identity or quantity.");
                    var departedQuantity = Element<Entry>("sr6-gear-quantity-" + coat);
                    var departedRemove = Element<Button>("sr6-gear-remove-" + coat);
                    IssuedPageLifecycle(page, "OnDisappearing");
                    departedQuantity.Text = "20"; departedRemove.IsEnabled = true; ((IButtonController)departedRemove).SendClicked();
                    await runtime.Presenter.LoadAsync(id, default);
                    var reopened = new Sr6CreationFoundationPage(runtime.Coordinator, gearMode: true);
                    await navigation.PushAsync(reopened, false);
                    await ui.BeginAsyncVoid(() => IssuedPageLifecycle(reopened, "OnAppearing"));
                    Require(IssuedElements(reopened).OfType<Entry>().Single(item => item.AutomationId == "sr6-gear-quantity-" + coat).Text == "3",
                        "Saved basket did not reopen unchanged.");
                    Require(!IssuedElements(reopened).OfType<Button>().Any(item => item.AutomationId == "sr6-foundation-confirm") && probe.Confirms == 3,
                        "Reopened basket exposed historical confirmation.");
                    ui.AssertHealthy();
                    Console.WriteLine("PASS SR6 gear phone " + method + " " + language);
                }
                finally { CultureInfo.CurrentUICulture = previousCulture; }
            }
            ...
            foreach (string language in new[] { "en", "de", "es" })
            {
                ...
                try
                {
                            ...
                            new("attributes", "A"), new("skills", "B"), new("resources", "C")])
                    {
                        PointBuy = method == "PointBuy" ? new(0, 0, 0, 0) : null,
                        Attributes = new(Sr6CreationAttributeIds.Ordered.Select(attribute => new Sr6CreationAttributeSpend(attribute, 0, 0)).ToArray()),
                        Skills = new([]), Karma = new([], [], 0)
                    };
                    var seed = (await runtime.Coordinator.PreviewSr6FoundationAsync(initial, selection)).Value!;
                    await runtime.Coordinator.ConfirmSr6FoundationAsync(seed, true);
                    var catalog = (await runtime.Coordinator.LoadSr6FoundationAsync()).Value!.QualityOptions!;
                    Require(!catalog.Any(row => row.Id is "built-tough-1" or "built-tough-2"), "Innate levels were offered for purchase.");
                    var page = new Sr6CreationFoundationPage(runtime.Coordinator, qualitiesMode: true);
                    var navigation = new NavigationPage(page);
                    var window = new Window(navigation);
                    using var alerts = new IssuedPageAlerts(page, window);
                    ...
                    T Element<T>(string key) where T : Element => IssuedElements(page).OfType<T>()
                        .Single(item => item.AutomationId == "sr6-foundation-" + key);
                    Task Click(string key) => ui.BeginAsyncVoid(() => ((IButtonController)Element<Button>(key)).SendClicked());
                    void Add(string key, bool replaces = false)
                    {
                        var option = catalog.Single(row => row.Id == key);
                        Element<Picker>("quality-catalog").SelectedIndex = Element<Picker>("quality-catalog").Items.IndexOf(Sr6CreationCopy.QualityName(option));
                        Require(Element<Button>("quality-add").IsEnabled && Element<Button>("quality-add").Text
                            == Sr6CreationCopy.Text(replaces ? "QualitiesReplace" : "QualitiesAdd"), "Replacement not explicit or eligible.");
                        Require(Element<Label>("quality-details").Text == Sr6CreationCopy.QualityValue(option), "Core rating/cost breakdown missing.");
                        ((IButtonController)Element<Button>("quality-add")).SendClicked();
                    }
                    Add("built-tough-3");
                    await Click("preview");
                    var oldConfirm = Element<Button>("confirm");
                    Add("built-tough-4", replaces: true);
                    oldConfirm.IsEnabled = true;
                    await ui.BeginAsyncVoid(() => ((IButtonController)oldConfirm).SendClicked());
                    Require(probe!.Confirms == 1, "Tier replacement retained stale confirmation.");
                    foreach (string quality in new[] { "analytical-mind", "ambidextrous", "hardening", "insomnia", "shaky-hands" }) Add(quality);
                    Require(IssuedElements(page).OfType<Button>().Count(row => row.AutomationId?.StartsWith("sr6-foundation-quality-remove-", StringComparison.Ordinal) == true) == 6,
                        "Tier replacement duplicated a choice.");
                    var fullPicker = Element<Picker>("quality-catalog");
                    Require(!fullPicker.Items.Contains(Sr6CreationCopy.QualityName(catalog.Single(row => row.Id == "dependents-1"))),
                        "Full selection still admits an unrelated seventh family.");
                    Add("built-tough-3", replaces: true);
                    await Click("preview");
                    await Click("confirm");
                    var stored = new FileWorkspaceStore(runtime.StateDirectory).Get(id).Value!;
                    var quote = stored.Document.AuxiliaryState.Sr6CreationFoundationDecisions!.Last().Preview;
                    var upgrade = quote.Qualities!.Values.Single(row => row.FamilyId == "built-tough");
                    Require(stored.ContentRevision == 3 && stored.SavedRevision == 3 && probe.Confirms == 2
                        && quote.Qualities.Values.Count == 6 && quote.Karma!.KarmaBudget == 37
                        && upgrade.Id == "built-tough-3" && upgrade.KarmaCost == 4
                        && upgrade.Rating == new Sr6CreationQualityRating(3, 2, 1, 4), "Innate upgrade/count/budget did not save exactly once.");
                    var oldAdd = Element<Button>("quality-add");
                    var oldPicker = Element<Picker>("quality-catalog");
                    IssuedPageLifecycle(page, "OnDisappearing");
                    oldPicker.SelectedIndex = 0; oldAdd.IsEnabled = true; ((IButtonController)oldAdd).SendClicked();
                    ...
                    await navigation.PushAsync(reopened, false);
                    await ui.BeginAsyncVoid(() => IssuedPageLifecycle(reopened, "OnAppearing"));
                    Require(IssuedElements(reopened).OfType<Button>().Count(row => row.AutomationId?.StartsWith("sr6-foundation-quality-remove-", StringComparison.Ordinal) == true) == 6
                        && IssuedElements(reopened).OfType<Label>().Any(row => row.Text == Sr6CreationCopy.QualityValue(upgrade)), "Rated upgrade did not reopen.");
                    Require(!IssuedElements(reopened).OfType<Button>().Any(row => row.AutomationId == "sr6-foundation-confirm") && probe.Confirms == 2,
                        "Reopened upgrade exposed historical confirmation.");
                    ui.AssertHealthy();
                    Console.WriteLine("PASS SR6 rated qualities phone " + method + " " + language);
                }
                finally { CultureInfo.CurrentUICulture = previousCulture; }
            }
