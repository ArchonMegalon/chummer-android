#!/usr/bin/env python3
"""Prove exact Chummer5 armor Home Node behavior on a real API 36 phone."""
...
PROFILE_TARGETS = {
    "CharacterCreate": {
        "armor_id": "53222222-5322-5322-5322-532222222222",
        "other_armor_id": "53333333-5333-5333-5333-533333333333",
        "gear_id": "53111111-5311-5311-5311-531111111111",
    ...
    "CharacterCareer": {
        "armor_id": "54222222-5422-5422-5422-542222222222",
        "other_armor_id": "54333333-5433-5433-5433-543333333333",
        "gear_id": "54111111-5411-5411-5411-541111111111",
...
def open_selected_armor(device: shared.Device, armor_id: str) -> None:
    ...
    device.wait(
        f"armor-home-node-open-{armor_id.replace('-', '')}",
        timeout=120,
        scroll=True,
        max_scrolls=36,
...
def open_home_node_page(device: shared.Device, armor_id: str) -> None:
    compact_id = armor_id.replace("-", "")
    open_selected_armor(device, armor_id)
    device.tap(f"armor-home-node-open-{compact_id}", timeout=60, scroll=True)
    device.wait(f"armor-home-node-page-{compact_id}", timeout=60)


def assert_toggle(device: shared.Device, armor_id: str, expected: bool) -> None:
    node = device.wait(f"armor-home-node-toggle-{armor_id.replace('-', '')}", timeout=60)
    observed = node.attributes.get("checked") == "true"
    if observed != expected:
        device.capture("armor-home-node-toggle-mismatch")
        raise RuntimeError(f"Armor Home Node switch was {observed!r}; expected {expected!r}")
    ...
    for payload in workspace_payloads(device):
        ...
        if (
            ...
            if len(true_flags) == (1 if enabled else 0):
                return
    device.capture("armor-home-node-workspace-not-persisted")
    raise RuntimeError(f"Armor Home Node invariant was not durable; observed {observed!r}")


def set_home_node(device: shared.Device, armor_id: str, enabled: bool) -> None:
    compact_id = armor_id.replace("-", "")
    open_home_node_page(device, armor_id)
    assert_toggle(device, armor_id, not enabled)
    device.tap(f"armor-home-node-toggle-{compact_id}", timeout=60)
    assert_toggle(device, armor_id, enabled)
    device.tap(f"armor-home-node-save-{compact_id}", timeout=240, scroll=True)
    device.wait(f"armor-home-node-open-{compact_id}", timeout=120, scroll=True)
...
def prove_profile(device: shared.Device, fixture: Path, profile: str) -> None:
    ...
    set_home_node(device, expected["armor_id"], True)
    assert_workspace_home_node(device, expected, True)
    open_home_node_page(device, expected["armor_id"])
    assert_toggle(device, expected["armor_id"], True)
    device.capture(f"armor-home-node-{profile.lower()}-enabled-after-reopen")
    ...
    open_home_node_page(device, expected["armor_id"])
    assert_toggle(device, expected["armor_id"], True)
    device.capture(f"armor-home-node-{profile.lower()}-enabled-after-process-restart")
    ...
    open_home_node_page(device, expected["armor_id"])
    assert_toggle(device, expected["armor_id"], False)
    device.capture(f"armor-home-node-{profile.lower()}-disabled-after-reopen")
    ...
    open_home_node_page(device, expected["armor_id"])
    assert_toggle(device, expected["armor_id"], False)
    device.capture(f"armor-home-node-{profile.lower()}-disabled-after-process-restart")
...
def main() -> int:
    ...
    parser.add_argument("--creation-runner", type=Path, default=fixtures / "creation-armor-home-node-e2e.chum5")
    parser.add_argument("--career-runner", type=Path, default=fixtures / "career-armor-home-node-e2e.chum5")
    ...
    source_paths = {
        ...
        "armorHomeNodePageSha256": android_root / "src" / "Chummer.Android" / "Native" / "ArmorHomeNodePage.cs",
        "collectionEditorPagesSha256": android_root / "src" / "Chummer.Android" / "Native" / "CollectionEditorPages.cs",
        "coordinatorSha256": android_root / "src" / "Chummer.Android" / "Native" / "RunnerSessionCoordinator.cs",
        "armorHomeNodeContractSha256": workspace_root / "chummer-presentation" / "Chummer.Presentation" / "Overview" / "ArmorHomeNodeEditRequest.cs",
    ...
    if not all(path.is_file() for path in source_paths.values()):
        ...
        raise RuntimeError(f"Armor Home Node E2E source graph is incomplete: {missing!r}")
    ...
    if api != "36":
        raise RuntimeError(f"Armor Home Node E2E requires API 36, got {api!r}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"armor Home Node E2E failed: {error}", file=sys.stderr)
        raise SystemExit(1)
