...
public sealed class Sr5CareerSkillGroupCheckpointStore
{
        ...
        try
        {
            lock (Gate)
            {
                if (checkpoint.Phase != Sr5CareerCheckpointPhase.Applying
                    ...
                    || !Sr5CareerSkillGroupCheckpointCas.From(checkpoint).Matches(current)
                    || !current.MatchesActionDraft(checkpoint.Draft)
                    || _authority is null
        ...
        out string blocker)
    {
        ...
        {
            blocker = "The skill-group mutation is still running; its Applying checkpoint remains locked.";
            return false;
        }
        try
        {
